...
const AllTeam = () => {
    ...
    return (
        <section className="container mx-0 sm:mx-auto">
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-y-8 gap-x-32 sm:gap-14 lg:gap-10 mx-16 justify-items-center" ref={cardsRef}>
                {
                    team.map((item, index) => (
                        <motion.div
                            key={index}
                            ...
                            animate={controls}
                            transition={{ duration: 0.2, delay: 0.5 + index * 0.3 }}
                            style={{ opacity: 0, y: 200 }}
                            className="w-44 h-60 sm:w-56 sm:h-72 rounded-xl shadow-xl border border-light-theme-purple justify-center transform transition duration-500 hover:scale-105"
                        >
